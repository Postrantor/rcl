...
{
public:
  void SetUp() { test_path /= "test_arguments"; }
...
bool are_known_ros_args(std::vector<const char *> argv)
{
  const int argc = static_cast<int>(argv.size());
  rcl_arguments_t parsed_args = rcl_get_zero_initialized_arguments();
  rcl_ret_t ret = rcl_parse_arguments(argc, argv.data(), rcl_get_default_allocator(), &parsed_args);
...
bool are_valid_ros_args(std::vector<const char *> argv)
{
  const int argc = static_cast<int>(argv.size());
  rcl_arguments_t parsed_args = rcl_get_zero_initialized_arguments();
  rcl_ret_t ret = rcl_parse_arguments(argc, argv.data(), rcl_get_default_allocator(), &parsed_args);
...
TEST_F(CLASSNAME(TestArgumentsFixture, RMW_IMPLEMENTATION), check_valid_vs_invalid_args)
{
  ...
  EXPECT_TRUE(are_valid_ros_args(
    {"--ros-args", "-p", "foo:=bar", "-r", "__node:=node_name", "--params-file",
     parameters_filepath.c_str(), "--log-level", "INFO", "--log-config-file", "file.config"}));
...
{
  const char * const argv[] = {"process_name",  "--ros-args", "/foo/bar:=", "-r",
                               "bar:=/fiz/buz", "}bar:=fiz",  "--",         "arg"};
  const int argc = sizeof(argv) / sizeof(const char *);
  rcl_arguments_t parsed_args = rcl_get_zero_initialized_arguments();
  rcl_allocator_t bad_alloc = get_failing_allocator();
  ...
  EXPECT_EQ(
    RCL_RET_INVALID_ARGUMENT, rcl_arguments_get_unparsed_ros(nullptr, allocator, &actual_unparsed));
  rcl_reset_error();
...
{
  const char * const argv[] = {"process_name",       "--ros-args", "-r",
                               "/foo/bar:=/fiz/buz", "--remap",    "foo:=/baz"};
  const int argc = sizeof(argv) / sizeof(const char *);
  rcl_arguments_t parsed_args = rcl_get_zero_initialized_arguments();
  rcl_ret_t ret;
...
{
  const char * const argv[] = {"process_name", "--ros-args", "/foo/bar:=", "-r", "bar:=/fiz/buz",
                               "-r",           "__ns:=/foo", "--",         "arg"};
  const int argc = sizeof(argv) / sizeof(const char *);
  rcl_arguments_t parsed_args = rcl_get_zero_initialized_arguments();
  rcl_ret_t ret;
...
static void * __return_null_on_zero_allocate(size_t size, void * state)
{
...
TEST_F(CLASSNAME(TestArgumentsFixture, RMW_IMPLEMENTATION), test_double_parse)
{
  ...
  OSRF_TESTING_TOOLS_CPP_SCOPE_EXIT({ EXPECT_EQ(RCL_RET_OK, rcl_arguments_fini(&parsed_args)); });
  ASSERT_EQ(
    RCL_RET_INVALID_ARGUMENT,
    rcl_parse_arguments(argc, argv, rcl_get_default_allocator(), &parsed_args));
  rcl_reset_error();
...
TEST_F(CLASSNAME(TestArgumentsFixture, RMW_IMPLEMENTATION), test_bad_remove_ros_args)
{
  ...
  EXPECT_EQ(
    RCL_RET_INVALID_ARGUMENT,
    rcl_remove_ros_arguments(NULL, &parsed_args, default_allocator, &nonros_argc, &nonros_argv));
  rcl_reset_error();

  EXPECT_EQ(
    RCL_RET_INVALID_ARGUMENT,
    rcl_remove_ros_arguments(argv, NULL, default_allocator, &nonros_argc, &nonros_argv));
  rcl_reset_error();
  ...
  EXPECT_EQ(
    RCL_RET_BAD_ALLOC,
    rcl_remove_ros_arguments(argv, &parsed_args, bad_alloc, &nonros_argc, &nonros_argv));
  rcl_reset_error();

  EXPECT_EQ(
    RCL_RET_INVALID_ARGUMENT,
    rcl_remove_ros_arguments(argv, &parsed_args, default_allocator, NULL, &nonros_argv));
  rcl_reset_error();

  EXPECT_EQ(
    RCL_RET_INVALID_ARGUMENT,
    rcl_remove_ros_arguments(argv, &parsed_args, default_allocator, &nonros_argc, NULL));
  rcl_reset_error();
  ...
  ret =
    rcl_remove_ros_arguments(NULL, &no_parsed_args, default_allocator, &nonros_argc, &nonros_argv);
  ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
...
TEST_F(CLASSNAME(TestArgumentsFixture, RMW_IMPLEMENTATION), test_bad_alloc_remove_ros_args)
{
  ...
  ret = rcl_remove_ros_arguments(argv, &parsed_args, bomb_alloc, &nonros_argc, &nonros_argv);
...
TEST_F(CLASSNAME(TestArgumentsFixture, RMW_IMPLEMENTATION), test_remove_ros_args)
{
  ...
  int nonros_argc = 0;
  const char ** nonros_argv = NULL;
  OSRF_TESTING_TOOLS_CPP_SCOPE_EXIT({ alloc.deallocate(nonros_argv, alloc.state); });
...
TEST_F(CLASSNAME(TestArgumentsFixture, RMW_IMPLEMENTATION), test_param_argument_single)
{
  ...
  EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
  OSRF_TESTING_TOOLS_CPP_SCOPE_EXIT({ rcl_yaml_node_struct_fini(params); });
...
TEST_F(CLASSNAME(TestArgumentsFixture, RMW_IMPLEMENTATION), test_param_argument_multiple)
{
  ...
  const char * const argv[] = {
    "process_name",     "--ros-args",  "--params-file", parameters_filepath1.c_str(), "-r",
    "__ns:=/namespace", "random:=arg", "--params-file", parameters_filepath2.c_str()};
  const int argc = sizeof(argv) / sizeof(const char *);
  rcl_ret_t ret;
  ...
  rcl_variant_t * param_value = rcl_yaml_node_struct_get("some_node", "int_param", params);
  ASSERT_TRUE(NULL != param_value);
  ASSERT_TRUE(NULL != param_value->integer_value);
...
TEST_F(CLASSNAME(TestArgumentsFixture, RMW_IMPLEMENTATION), test_param_overrides)
{
  ...
  const char * const argv[] = {
    ...
    "string_param:=bar",
    "-p",
    "some.bool_param:=false",
    "-p",
    "some_node:int_param:=4"};
  const int argc = sizeof(argv) / sizeof(const char *);
  ...
  rcl_variant_t * param_value = rcl_yaml_node_struct_get("/**", "string_param", params);
  ASSERT_TRUE(NULL != param_value);
  ASSERT_TRUE(NULL != param_value->string_value);
...
TEST_F(CLASSNAME(TestArgumentsFixture, RMW_IMPLEMENTATION), test_parse_with_internal_errors)
{
  ...
  const char * const argv[] = {
    ...
    RCL_ROS_ARGS_FLAG,
    RCL_PARAM_FILE_FLAG,
    parameters_filepath1.c_str(),
    RCL_REMAP_FLAG,
    "that_node:foo:=baz",
    RCL_REMAP_FLAG,
    "foo:=bar",
    RCL_PARAM_FILE_FLAG,
    parameters_filepath2.c_str(),
    RCL_REMAP_FLAG,
    "__name:=my_node",
    RCL_REMAP_FLAG,
    "__ns:=/my_ns",
