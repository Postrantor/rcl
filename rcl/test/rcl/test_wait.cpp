...
TEST_F(CLASSNAME(WaitSetTestFixture, RMW_IMPLEMENTATION), test_failed_resize)
{
  ...
  rcl_ret_t ret = rcl_wait_set_init(&wait_set, 1, 1, 1, 1, 1, 0, context_ptr, allocator);
  EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
...
TEST_F(CLASSNAME(WaitSetTestFixture, RMW_IMPLEMENTATION), zero_timeout_overrun_timer)
{
  ...
  rcl_timer_t timer = rcl_get_zero_initialized_timer();
  ret = rcl_timer_init(&timer, &clock, this->context_ptr, 0, nullptr, rcl_get_default_allocator());
  EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
  OSRF_TESTING_TOOLS_CPP_SCOPE_EXIT({
    ret = rcl_timer_fini(&timer);
    EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
...
TEST_F(CLASSNAME(WaitSetTestFixture, RMW_IMPLEMENTATION), canceled_timer)
{
  ...
  ret = rcl_timer_init(
    &canceled_timer, &clock, this->context_ptr, RCL_MS_TO_NS(1), nullptr,
    ...
    return [=, &test_set]() {
      while (test_set.wake_count < count_target) {
        ...
        while (wake_try_count < retry_limit) {
          ...
          EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
          ret =
            rcl_wait_set_add_guard_condition(&test_set.wait_set, &test_set.guard_condition, NULL);
          EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
...
TEST_F(CLASSNAME(WaitSetTestFixture, RMW_IMPLEMENTATION), guard_condition)
{
  ...
  std::thread trigger_thread([&p, &guard_cond, &trigger_diff]() {
    ...
    trigger_diff =
      std::chrono::duration_cast<std::chrono::nanoseconds>(after_trigger - before_trigger);
    p.set_value(ret);
  });
  auto f = p.get_future();
...
TEST_F(CLASSNAME(WaitSetTestFixture, RMW_IMPLEMENTATION), add_with_index)
{
  ...
  for (size_t i = 0u; i < kNumEntities; ++i) {
    ...
    ret = rcl_wait_set_add_guard_condition(&wait_set, &guard_conditions[i], &guard_condition_index);
    EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
...
TEST_F(CLASSNAME(WaitSetTestFixture, RMW_IMPLEMENTATION), wait_set_valid_arguments)
{
  ...
  // nullptr failures
  ret = rcl_wait_set_init(nullptr, 1, 1, 1, 1, 1, 0, context_ptr, rcl_get_default_allocator());
  EXPECT_EQ(RCL_RET_INVALID_ARGUMENT, ret);
  EXPECT_TRUE(rcl_error_is_set());
  rcl_reset_error();

  ret = rcl_wait_set_init(&wait_set, 1, 1, 1, 1, 1, 0, nullptr, rcl_get_default_allocator());
  EXPECT_EQ(RCL_RET_INVALID_ARGUMENT, ret);
  EXPECT_TRUE(rcl_error_is_set());
  rcl_reset_error();
  ...
  ret = rcl_wait_set_init(&wait_set, 1, 1, 1, 1, 1, 0, context_ptr, zero_init_allocator);
  EXPECT_EQ(RCL_RET_INVALID_ARGUMENT, ret);
  EXPECT_TRUE(rcl_error_is_set());
  rcl_reset_error();
...
TEST_F(CLASSNAME(WaitSetTestFixture, RMW_IMPLEMENTATION), wait_set_failed_init)
{
  ...
  auto mock = mocking_utils::patch_and_return("lib:rcl", rmw_create_wait_set, nullptr);
  rcl_ret_t ret =
    rcl_wait_set_init(&wait_set, 1, 1, 1, 1, 1, 0, context_ptr, rcl_get_default_allocator());
...
TEST_F(CLASSNAME(WaitSetTestFixture, RMW_IMPLEMENTATION), wait_set_failed_fini)
{
  ...
  ASSERT_EQ(RCL_RET_OK, ret);
  {
    ...
    auto mock = mocking_utils::inject_on_return("lib:rcl", rmw_destroy_wait_set, RMW_RET_ERROR);
