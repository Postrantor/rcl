  ...
  rcl_lifecycle_transition_map_t * transition_map, const rcutils_allocator_t * allocator)
{
  ...
  // 注册关闭状态
  {
    rcl_lifecycle_state_t rcl_state_shuttingdown = {
      "shuttingdown", lifecycle_msgs__msg__State__TRANSITION_STATE_SHUTTINGDOWN, valid_transitions,
      valid_transition_size};
  ...
  // 注册停用状态
  {
    rcl_lifecycle_state_t rcl_state_deactivating = {
      "deactivating", lifecycle_msgs__msg__State__TRANSITION_STATE_DEACTIVATING, valid_transitions,
      valid_transition_size};
  ...
  rcl_lifecycle_transition_map_t * transition_map, const rcutils_allocator_t * allocator)
{
  ...
  // 获取未配置状态
  rcl_lifecycle_state_t * unconfigured_state =
    rcl_lifecycle_get_state(transition_map, lifecycle_msgs__msg__State__PRIMARY_STATE_UNCONFIGURED);
  // 获取非活动状态
  ...
  // 注册从cleaniningup状态到errorprocessing状态的转换
  {
    ...
    // 将定义的转换注册到transition_map中
    ret =
      rcl_lifecycle_register_transition(transition_map, rcl_transition_on_cleanup_error, allocator);
    // 判断注册是否成功
    if (ret != RCL_RET_OK) {
      return ret;
    }
 ...
 */
  {
    ...
    // 将转换注册到转换映射中
    ret =
      rcl_lifecycle_register_transition(transition_map, rcl_transition_active_shutdown, allocator);
 ...
 */
  ...
  /**
 * @brief 注册从 errorprocessing 状态到 unconfigured 状态的转换
 */
  {
    ...
    // 注册转换
    ret =
      rcl_lifecycle_register_transition(transition_map, rcl_transition_on_error_success, allocator);
    // 检查注册结果
    if (ret != RCL_RET_OK) {
      return ret;
    }
  ...
  /**
 * @brief 注册从 errorprocessing 状态到 finalized 状态的转换（失败情况）
 */
  {
    ...
    // 注册转换
    ret =
      rcl_lifecycle_register_transition(transition_map, rcl_transition_on_error_failure, allocator);
    // 检查注册结果
    if (ret != RCL_RET_OK) {
      return ret;
    }
  ...
  /**
 * @brief 注册从 errorprocessing 状态到 finalized 状态的转换（错误情况）
 */
  {
 ...
 * @param[out] fail_error_message 初始化失败时的错误信息
 * @param[out] fini_error_message 释放资源失败时的错误信息
 */
  // 如果有错误设置
  if (rcl_error_is_set()) {
    // 复制错误信息字符串
